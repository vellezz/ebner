...
- **Aliens are not humans in costume.** Give a world its physics, climate, gravity, day length, biology.
- **A world is conditions first, not a conceit.** Dust that clogs everything, gravity that hurts after two days, bad food, a port where things go missing. A place does not need a governing rule, and four in a row had one: voting decides everything, time is piped, ownership needs unbroken touch, heavy gas pools so speech costs breath. Requiring the premise to be physical rather than clerical did not fix that — it swapped a regulation for a theorem, and the reader's word for the result was still "abstract". One rule per civilisation is allowed but rare; when it is there it stays background, visible through what it costs someone, and it is never the entry's subject.
- **The subject is work, danger, money or a person.** An entry whose plot is a system being figured out — test, result, inference, consequence — is a puzzle with a solution rather than a day in a life, however concrete its details.
